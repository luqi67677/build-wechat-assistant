...
| Hermes | 当前 Profile 对应的 `<HERMES_HOME>/skills/` | 先让 Agent 用 `hermes -p <Profile> config path` 确认目标 Profile 的实际根目录，再克隆到其中的 `skills/build-wechat-assistant`；不要把 `~/.hermes/skills/` 当作所有 Profile 的固定路径 |
