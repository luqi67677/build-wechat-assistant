...
最后核验：2026-08-15。官方 tag v2026.8.3 对应 Hermes v0.20.0；V0.5 已在该官方干净 tag 的隔离临时根通过 CLI 契约、扫码前工具收缩、Cron 正反路径，以及知识库、飞书、编程三个 MCP 的保存与 stdio 重连检查，没有使用模型秘密、没有生成二维码，也没有安装服务。新安装仍必须对目标机实际 Hermes 绝对路径重跑 `scripts/check_hermes_cli_contract.py`；Windows、Linux、真实模型、真实扫码和开机恢复仍需各自平台证据。执行前同时读取当前官方文档、`hermes --version` 和对应 `--help`。
